...
const std::vector<const char*>& OnnxEngine::input_name() const noexcept
{
    return input_names_;
}

const std::vector<const char*>& OnnxEngine::output_name() const noexcept
{
    return output_names_;
}

const std::vector<int64_t>& OnnxEngine::input_shape() const noexcept
{
    return input_shape_;
}

const std::vector<std::vector<int64_t>>& OnnxEngine::output_shapes() const noexcept
{
    return output_shapes_;
}

const std::vector<InferenceOutput>& OnnxEngine::run(const std::vector<float>& input_data)
{
...
InferenceOutput OnnxEngine::merge_split_outputs(std::vector<InferenceOutput> outputs)
{
    InferenceOutput merged;
    if (outputs.empty()) {
        return merged;
    }
    merged = std::move(outputs[0]);
    int64_t sum_dim1 = merged.shape[1];
    const int64_t dim2 = merged.shape[2];
    for (std::size_t i = 1; i < outputs.size(); ++i) {
        merged.data.insert(merged.data.end(),
            std::make_move_iterator(outputs[i].data.begin()),
            std::make_move_iterator(outputs[i].data.end()));
        sum_dim1 += outputs[i].shape[1];
    }
    merged.shape = {1, sum_dim1, dim2};
    return merged;
}
